...
namespace ConsoleApp5
{
    ...
    class SOKOBAN
    {
        ...
        public static Tile[,] CrearMapa(string[] infoMapa)
        {
            ...
            //Se crea un array de tiles , en las cuales (filas,columnas)
            ...
            for (int f = 0; f < rv.GetLength(0); f++)
            {
                ...
                {
                    ///Remplazar por excepcion
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("Alguno de los elementos de infoMapa es de diferente largo que el resto");
                ...
                for (int c = 0; c < rv.GetLength(1); c++)
                {
                    switch (infoMapa[f][c])
                    {
                        ...
                        case 'I':
                            ...
                            break;
                        default:
                            //Cambiar por la excepcion
                            Console.WriteLine("Se encontro un caracter que no corresponde con los enumerados como validos");
                            break;
                    }
            ...
            {
                Console.WriteLine("Cantidad de Botones es diferente de cantidad de  cajas");
            }
            if (cantCajas == 0 || cantBotones == 0)
            {
                Console.WriteLine("No hay botones o paredes en el Mapa");
            }
            if (cantI != 1)
            {
                Console.WriteLine("Cantidad Jugadores Inadecuada");
            }
        ...
        public static bool[,] CrearCajas(Tile[,] mapa)
        {
            ...
            if (mapa==null)
            {
                Console.WriteLine("mapa es null");
            }
        ...
        public static void InicializarJugador(Tile[,] mapa, ref int filaJugador, ref int columnaJugador)
        {
            if (mapa==null)
            {
                Console.WriteLine("Mapa es Nulo");
            }
        ...
        public static bool EjecutarMovimiento(Direccion dir, Tile[,] mapa, bool[,] cajas, ref int filaJugador, ref int columnaJugador)
        {
            ...
            {
                Console.WriteLine("Alguna de las matrices es null");
            }

            if (mapa.GetLength(0)!=cajas.GetLength(0) || mapa.GetLength(1) != cajas.GetLength(1))
            {
                Console.WriteLine("El tamaño de las dimensiones de mapa y cajas son diferente");
            }

            if (filaJugador>mapa.GetLength(0) ||  columnaJugador>mapa.GetLength(1) || filaJugador<0 || columnaJugador<0 )
            {
                Console.WriteLine("filaJugador o columnaJugador se encuentra fuera de los limites de mapa");
            }
        ...
        public static bool EsVictoria(Tile[,]mapa, bool[,]cajas)
        {
            for (int f = 0; f < mapa.GetLength(0); f++)
            {
                for (int c = 0; c < mapa.GetLength(1); c++)
                {
                    if (mapa[f,c]==Tile.Boton && cajas[f,c]==false)
                    {
                        return false;
                    }
                }
        ...
        /// - El jugador debe dibujarse como '&' de color ConsoleColor.Yellow--ok
        /// - Las paredes como '+' de color ConsoleColor.DarkGray. --ok
        /// - Las cajas deben ser dibujadas como '#' ConsoleColor.White 
        ///   siempre que no se encuentren ubicadas sobre un boton, 
        ///   si estan sobre un boton, seran ConsoleColor.Green.
        ///   
        /// - Los botones como '*' de color ConsoleColor.Red,
        ///   siempre y cuando no haya una caja sobre el, si hay una caja o un jugador en la misma posicion, 
        ///   el boton no se dibuja.
            ...
            for (int f = 0; f < mapa.GetLength(0); f++)
            {
                for (int c = 0; c < mapa.GetLength(1); c++)
                {
                    ...
                    if ((mapa[f, c]) == Tile.InicioJugador)
                    {
                        ...
                        {
                            //if (mapa[filaJugador,columnaJugador]==Tile.Boton)
                            //{

                            //}
                            mapa[f, c] = Tile.Piso;
                            mapa[filaJugador, columnaJugador] = Tile.InicioJugador;
                            Limpiar();
                            c = 0;
                            f = 0;                            
                        }
                    ...
                    if ((mapa[f, c]) == Tile.Boton)
                    {
                        if (cajas[f, c]==false || mapa[f, c]==Tile.InicioJugador)
                        {
                    ...
                    if ((mapa[f, c]) == Tile.Pared)
                    {
                        Console.ForegroundColor = ConsoleColor.DarkGray;
                        Console.Write("+");
                    }
                    if ((mapa[f, c]) == Tile.Piso)
                    {
                        Console.Write(" ");
                    }
